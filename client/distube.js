...
distube.on(`playSong`, (queue, song) => {
  try {
    queue.textChannel.send({
      content: `> <:emoji_66:1089046438484389898>  *Playing*: **${song.name}** - \`(${song.formattedDuration})\``
    });
...
distube.on("addSong", (queue, song) => {
  try {
  queue.textChannel.send({ content: `> <:emoji_66:1089046438484389898>  *Added* \`${song.name}\` - \`(${song.formattedDuration})\` **to the queue by ${song.user}**` }),
...
distube.on("addList", (queue, playlist) => {
  try {
    queue.textChannel.send({ content: `> <:emoji_66:1089046438484389898>  *Added* \`${playlist.name}\` __**playlist**__ **\`(${playlist.songs.length} songs)\` ** to queue**` }),
